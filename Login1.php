...
if (isset($_POST['btnlogin'])) {
    ...
    if ($error_msg == '') {
        ...
        if ($loginas == "admin") {
            ...
            //login for patient
            $query = "select name from regestration1 where email='$email' and password = '$password'";
            $result = mysql_query($query);
            while ($row = mysql_fetch_array($result)) {
                $name = $row ['name'];
                if ($name != '') {
            ...
            //login for doctor
            $query = "select id,name from company where username='$email' and password = '$password'";
            $result = mysql_query($query);
            while ($row = mysql_fetch_array($result)) {
                $name = $row ['id'];
                if ($name != '') {
